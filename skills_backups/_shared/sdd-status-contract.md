...
- When the session artifact store is `openspec` or `hybrid`, prefer the native OpenCode slash commands `/sdd-status [change] --cwd <repo> --json --instructions` for read-only status and `/sdd-continue [change] --cwd <repo>` for dispatcher output. The native engine reads only OpenSpec file artifacts and always emits `artifactStore: openspec`; it cannot observe tonymem-backed changes. Treat native status as authoritative only when the selected artifact store is `openspec` or `hybrid`. When the selected store is `tonymem`, do not invoke the native dispatcher at all — resolve status from tonymem (`mem_search` + `mem_get_observation` on the change topic keys) using the manual status schema below, and disregard any `blocked`, `Active OpenSpec change not found`, or `nextRecommended: sdd-new` it emits for a tonymem change that exists.
...
- If the native dispatcher is unavailable, fall back to this prompt contract and the manual status schema below. Manual fallback status MUST stay shape-compatible with native dispatcher JSON even when values are reconstructed manually.
