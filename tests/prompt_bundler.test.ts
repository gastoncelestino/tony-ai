...
function createFixture(): string {
  ...
  mkdirSync(join(fixtureRoot, 'prompts/agents/includes'), {
    recursive: true,
  });
  mkdirSync(join(fixtureRoot, 'prompts/sdd'), {
    recursive: true,
  });
  mkdirSync(join(fixtureRoot, 'skills/_shared'), {
    recursive: true,
  });
...
describe('prompt bundler', () => {
  ...
  test('fails with the full chain for a missing include', () => {
    ...
    expect(() => buildOrchestrator(fixtureRoot)).toThrow(
      /include not found.*missing\.md/,
    );
  });
  ...
  test('writes deterministic generated outputs and detects drift', () => {
    ...
    expect(checkGenerated(fixtureRoot)).not.toEqual([]);

    expect(
      checkGenerated(fixtureRoot).some((item) => item.includes('stale')),
    ).toBe(true);
  ...
  test('--phase rebuilds the full tree and keeps the manifest current', () => {
    writeAll(fixtureRoot);

    writeFixtureFile(
      'prompts/agents/includes/nested/b.md',
      'B changed by phase build\n',
    );
  ...
  test('records dependency and bundle hashes', () => {
    ...
    const expected = createHash('sha256')
      .update(built.orchestrator.content, 'utf8')
      .digest('hex');

    expect(built.manifest.bundle_sha256).toBe(expected);

    expect(built.manifest.dependencies.map((item) => item.path)).toContain(
      'prompts/agents/includes/a.md',
    );
  });
});
...
describe('repository prompt bundle contract', () => {
  ...
  test('all manifest phases have generated bundles and generated config pointers', () => {
    ...
    for (const phase of Object.keys(built.manifest.phases)) {
      expect(config.agent[phase]?.prompt).toBe(
        `{file:./prompts/generated/phases/${phase}.md}`,
      );
    }
  });
  ...
  test('source prompts use bundler directives instead of nested native config references', () => {
    const source = readFileSync(
      join(repoRoot, 'prompts/agents/tony-orchestrator.md'),
      'utf8',
    );
