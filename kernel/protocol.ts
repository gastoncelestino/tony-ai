...
export function isKernelTask(value: unknown): value is KernelTask {
  ...
  return typeof task.id === "string" && typeof task.description === "string" &&
    typeof task.phase === "string" && KERNEL_PHASES.includes(task.phase as KernelPhase) &&
    Array.isArray(task.dependencies) && task.dependencies.every((id) => typeof id === "string") &&
    (task.files === undefined || (Array.isArray(task.files) && task.files.every((file) => typeof file === "string")))
}
...
export function isKernelContext(value: unknown): value is KernelContext {
  ...
  return typeof context.phase === "string" && typeof context.status === "string" &&
    Array.isArray(context.tasks) && context.tasks.every(isKernelTask) &&
...
function isKernelBoundaryResponse(value: unknown): value is KernelBoundaryResponse {
  if (!value || typeof value !== "object") return false
  const response = value as Record<string, unknown>
  if (response.allowed === false) return response.decision === "blocked" && typeof response.reason === "string" && response.execution_order === null
  ...
  return typeof valueOrder.task_id === "string" && typeof valueOrder.description === "string" &&
    typeof valueOrder.phase === "string" && KERNEL_PHASES.includes(valueOrder.phase as KernelPhase) &&
