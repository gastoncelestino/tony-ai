...
function bootstrapPrompt(
  ...
  return `Objective:\n${prompt || description || "(not provided)"}\n\nReturn only a machine-readable task graph wrapped in <task_result> tags.`
}
...
export async function authorizeExecution(
  ...
  // D: the runtime resolves the next Action Plan. The model-provided task
  // description, phase and agent are treated as untrusted input and replaced.
  const plan = resolveActionPlan(provided.context)
  if (plan.action === "done") {
    throw new KernelDoneError(plan.reason)
  }

  input.args.description = plan.objective
  input.args.prompt = actionPlanPrompt(plan)
  input.args.subagent_type = plan.agent
  delete input.args.command
  ...
  return {
    allowed: true,
    reason: `${response.reason}; action_plan=${plan.task_id}:${plan.agent}`,
    order: validateOrder(
      adapted.request,
