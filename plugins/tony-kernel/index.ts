...
export interface KernelClientLike {
  ...
  recordPhaseCompletion(phase: string, artifacts: Array<{ kind: string; path: string; store: string; hash?: string }>, evidence?: unknown[]): Promise<{ decision: string; allowed: boolean; reason: string; missing_artifacts: string[]; missing_evidence: string[] }>
  checkScope(gitDiff: string, allowedFiles: string[]): Promise<{ decision: string; allowed: boolean; reason: string; scope_violations: string[] }>
  getStatus(): Promise<Record<string, unknown>>
}

export class KernelBlockedError extends Error {
  constructor(message: string) { super(message); this.name = "KernelBlockedError" }
}

export class KernelUnavailableError extends Error {
  constructor(message: string) { super(message); this.name = "KernelUnavailableError" }
}
...
class KernelClient implements KernelClientLike {
  ...
  private async runKernelCommand(args: string[]): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      proc.on("close", (code) => {
        if (code === 0) {
          try { resolve(JSON.parse(stdout.trim())) } catch { resolve({ success: true, output: stdout.trim() }) }
        } else reject(new Error(`Kernel error (${code}): ${stderr}`))
      })
      proc.on("error", (err) => reject(new Error(`Kernel spawn error: ${err.message}`)))
    })
  }
  ...
  async authorizeTool(tool: string): Promise<RuntimeAuthorizationResult> {
    ...
    return { allowed: result.allowed === true, reason: typeof result.reason === "string" ? result.reason : "" }
  }

  async authorizeCommand(command: string): Promise<RuntimeAuthorizationResult> {
    const result = await this.runKernelCommand(["authorize_command", command])
    return { allowed: result.allowed === true, reason: typeof result.reason === "string" ? result.reason : "" }
  }
  ...
  async recordPhaseCompletion(phase: string, artifacts: Array<{ kind: string; path: string; store: string; hash?: string }>, evidence: unknown[] = []): Promise<any> { const result = await this.runKernelCommand(["record_phase_completion", phase, JSON.stringify(artifacts), JSON.stringify(evidence)]); return { ...result, allowed: result.allowed === true } }
  async checkScope(gitDiff: string, allowedFiles: string[]): Promise<any> { const result = await this.runKernelCommand(["check_scope", gitDiff, JSON.stringify(allowedFiles)]); return { ...result, allowed: result.allowed === true } }
  async getStatus(): Promise<Record<string, unknown>> { return this.runKernelCommand(["status"]) }
}
...
export function isKernelPhase(subAgent: string): boolean { return FSM_PHASES.has(subAgent) }
...
export function derivePhase(args: Record<string, unknown>): string | null {
  if (typeof args.phase === "string" && args.phase.length > 0) return args.phase.replace(/^sdd-/, "")
  if (typeof args.subagent_type === "string") { const subAgent = args.subagent_type; if (isKernelPhase(subAgent)) return subAgent.replace(/^sdd-/, ""); if (isNonFsmAgent(subAgent)) return null }
  throw new KernelBlockedError("[Tony Kernel] Unable to derive phase from task delegation")
}
...
export async function taskExecuteBeforeHook(input: DelegationInput, output: DelegationOutput): Promise<void> {
  ...
  try {
    ...
    if (!toolResult.allowed) throw new KernelBlockedError(`[Tony Kernel] Tool authorization blocked "${authorization.tool}": ${toolResult.reason}`)
    if (authorization.command !== null) {
  ...
  try {
    ...
    throw new KernelUnavailableError(kernelErrorMessage("Delegation gate", error))
  }
}
...
export async function taskExecuteAfterHook(input: { sessionID: string; tool: string; arguments: Record<string, unknown> }, output: unknown): Promise<void> {
  ...
  if (evidence) {
    try { const client = await getKernelClient(); const result = await client.completeTask(evidence.task_id, [evidence.evidence]); if (!result.allowed) throw new KernelBlockedError(`[Tony Kernel] Task completion rejected for "${evidence.task_id}": ${result.reason}`); return }
    catch (error) { if (error instanceof KernelBlockedError) throw error; throw new KernelUnavailableError(kernelErrorMessage("Task evidence", error)) }
  }
  if (input.tool !== "Task") return
  const args = observation.arguments; const phase = derivePhase(args); if (phase === null) return
  try {
    const outputText = typeof observation.result === "string" ? observation.result : JSON.stringify(observation.result)
    ...
    const client = await getKernelClient()
    if (gitDiff.length > 0) { const scopeResult = await client.checkScope(gitDiff, allowedFiles); if (!scopeResult.allowed) throw new KernelBlockedError(scopeViolationMessage(phase, scopeResult)) }
    ...
    const status = await client.getStatus(); const currentPhase = status.current_phase
    if (currentPhase !== phase) throw new KernelBlockedError(`[Tony Kernel] Post-phase validation failed: kernel state is ${currentPhase}, expected ${phase} after completion`)
  } catch (error) {
    if (error instanceof KernelBlockedError || error instanceof KernelUnavailableError) throw error
    throw new KernelUnavailableError(kernelErrorMessage("Phase completion", error))
  }
}

const TonyKernelPlugin: Plugin = {
  name: "tony-kernel", version: "1.0.0",
  description: "Tony Kernel — Deterministic hook for phase transitions, evidence tracking, and scope enforcement",
  hooks: { "tool.execute.before": taskExecuteBeforeHook, "tool.execute.after": taskExecuteAfterHook },
  async onLoad() { console.log("[tony-kernel] Plugin loaded, kernel integration active") },
  async onUnload() {},
}
