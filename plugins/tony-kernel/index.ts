...
class KernelClient implements KernelClientLike {
  ...
  private async runKernelCommand(args: string[]): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      proc.on("close", (code) => { if (code === 0) { try { resolve(JSON.parse(stdout.trim())) } catch { resolve({ success: true, output: stdout.trim() }) } } else reject(new Error(`Kernel error (${code}): ${stderr}`)) })
...
export async function taskExecuteBeforeHook(input: DelegationInput, output: DelegationOutput): Promise<void> {
  ...
  try {
    ...
    if (authorization.command !== null && client.authorizeCommand) {
...
export async function taskExecuteAfterHook(input: { sessionID: string; tool: string; arguments: Record<string, unknown> }, output: unknown): Promise<void> {
  const observation = observeToolExecution(input, output); const evidence = observationToEvidence(observation)
  if (evidence) { try { const client = await getKernelClient(); const result = await client.completeTask(evidence.task_id, [evidence.evidence]); if (!result.allowed) throw new KernelBlockedError(`[Tony Kernel] Task completion rejected for "${evidence.task_id}": ${result.reason}`); return } catch (error) { if (error instanceof KernelBlockedError) throw error; throw new KernelUnavailableError(kernelErrorMessage("Task evidence", error)) } }
  if (input.tool !== "Task") return
...
const TonyKernelPlugin: Plugin = { name: "tony-kernel", version: "1.0.0", description: "Tony Kernel — Deterministic hook for phase transitions, evidence tracking, and scope enforcement", hooks: { "tool.execute.before": taskExecuteBeforeHook, "tool.execute.after": taskExecuteAfterHook }, async onLoad() { console.log("[tony-kernel] Plugin loaded, kernel integration active") }, async onUnload() {} }
