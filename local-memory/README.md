...
Memoria persistente local para el orquestador SDD de OpenCode —
observaciones con `topic_key` que hacen *upsert*, scoping por proyecto,
búsqueda full-text, expuesto como tools MCP. Características:

- **100% tuyo**: un solo archivo Python (`server.py`, stdlib puro — nada de
  `pip install`) + un `memory.db` de SQLite que se crea solo.
- **100% local**: no hay servidor, no hay cloud sync, no hay telemetría.
  Todo vive en tu disco.
- **Sin instalador**: copiás la carpeta `local-memory/` donde quieras y
  apuntás OpenCode a `server.py`. Listo.
...
> **Nota (fork Tony-AI)**: este README describe la versión original de
> 4 tools. En el fork de Gentle-AI → Tony-AI, `server.py` se extendió con
> 4 tools más (`mem_context`, `mem_session_summary`, `mem_suggest_topic_key`,
> `mem_save_prompt`) porque el `AGENTS.md` de Gentle-AI y algunos de sus
> skills los llaman por nombre. Se agregaron ahí — reales, no stubs — en
> vez de tocar esos archivos.

Los 4 originales, que usa el AGENTS.md del orquestador SDD (`sdd-explore`,
`sdd-propose`, `sdd-apply`, etc.) para leer/escribir artefactos entre fases:
...
`tonymem` implementa **solo el contrato que el orquestador SDD realmente
usa** (los 4 tools de arriba), sin:
