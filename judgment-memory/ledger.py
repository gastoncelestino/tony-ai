...
# ---------------------------------------------------------------------------
# Configuration — same env var naming convention as code-index/core.py
# ---------------------------------------------------------------------------
...
def _record_to_row(record: dict) -> tuple:
    ...
    return (
        record["execution_id"],
        record.get("project", "default"),
        record["task"],
        judge_a.get("model"),
        judge_a.get("decision"),
        judge_b.get("model"),
        judge_b.get("decision"),
        record.get("agreement"),
        record.get("winner"),
        record.get("confidence"),
        record["final"],
        record.get("fix"),
        record.get("lesson"),
        record.get("source_lineage_id"),
    )


def save_judgment(record: dict, point_id: str = None) -> dict:
    """Validate-lite + upsert a judgment record by (project, execution_id).

    Re-saving the same execution_id updates in place — the ledger has one
    row per Judgment Day lineage outcome, not per attempt.
    """
        ...
        if row:
            conn.execute(
                """UPDATE judgments SET
                    ...
                    point_id=?, created_at=?
                   WHERE id=?""",
                (*values[2:], point_id or row["point_id"], ts, row["id"]),
            )
            conn.commit()
        ...
        cur = conn.execute(
            """INSERT INTO judgments
               (execution_id, project, task, judge_a_model, judge_a_decision, judge_b_model, judge_b_decision,
                agreement, winner, confidence, final, fix, lesson, source_lineage_id, point_id, created_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (*values, point_id, ts),
        )
        conn.commit()
...
# ---------------------------------------------------------------------------
# Normalize — turn a structured record into one embeddable passage
# ---------------------------------------------------------------------------

def normalize(record: dict) -> str:
    """Flatten a judgment record into the text that gets embedded.

    Deliberately lesson-forward: `lesson` is what a future task actually
    wants to match against ("check execution plan before optimization"),
    so it's stated twice (once plain, once as the closing takeaway) to
    weight it in the embedding without needing a weighted-embedding API.
    """
    judge_a = record.get("judge_a") or {}
    judge_b = record.get("judge_b") or {}
    parts = [
        f"task: {record.get('task', '')}",
        f"outcome: {record.get('final', '')}",
    ]
...
# ---------------------------------------------------------------------------
# Qdrant REST client (same contract as code-index/core.py)
# ---------------------------------------------------------------------------
...
def point_id_for(project: str, execution_id: str) -> str:
    """Deterministic UUID so re-indexing the same execution_id upserts."""
...
def record_judgment(record: dict, embed_model: str = EMBED_MODEL, embeddings_url: str = EMBEDDINGS_URL,
    ...
    try:
        ...
        qdrant_upsert(
            coll,
            [{"id": pid, "vector": vec, "payload": {**record, "project": project}}],
            base_url=qdrant_url,
        )
...
def recall(task: str, project: str = "default", limit: int = 5,
    ...
    searches this project's Qdrant collection of past lessons.
    """
    coll = collection_name(project)
    try:
...
def _cli() -> None:
    ...
    if args.cmd == "record":
        with open(args.file, "r", encoding="utf-8") as f:
            record = json.load(f)
        result = record_judgment(record, index=not args.no_index)
        print(json.dumps(result, indent=2, ensure_ascii=False))
    elif args.cmd == "recall":
        print(json.dumps(recall(args.task, project=args.project, limit=args.limit), indent=2, ensure_ascii=False))
    elif args.cmd == "history":
        print(json.dumps(
            history(project=args.project, limit=args.limit, all_projects=args.all_projects),
