...
- **tonymem**: Read `sdd/{change-name}/proposal` (required) and `sdd/{change-name}/spec` (optional — may not exist if running in parallel with sdd-spec). Save as `sdd/{change-name}/design`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`.
- **hybrid**: Follow BOTH conventions — persist to tonymem AND write `design.md` to filesystem. Retrieve dependencies from tonymem (primary) with filesystem fallback.
...
**IF mode is `tonymem` or `none`:** Do NOT create any `openspec/` directories or files. Compose the design content in memory — you will persist it in Step 4.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/design.md` (openspec/hybrid) | tonymem `sdd/{change-name}/design` (tonymem) | inline (none)
