...
Pytest es recomendado pero NO obligatorio. Si no está disponible, el Makefile usa automáticamente `tools/run-python-tests.py`.
...
CI ejecuta Python 3.10, 3.11, 3.12.
...
Python 3.10, 3.11, 3.12 con Bun 1.3.14.

Para cada versión:
1. `make check-test-discovery`
2. `make test`
3. Si Python 3.12: `make coverage`

### Artifacts

- `coverage.xml`
- `coverage-contexts.json`
- `coverage-bun/lcov.info`

### Job separado: Docker build

Se compila la imagen Docker.

### Nota importante

CI **no necesita Ollama ni Qdrant** para la suite local. Solo para smoke tests manuales.
...
| `make coverage-python` | Coverage Python | — |
...
Un PR es **crítico** si toca Kernel, MCP servers, configuración, infraestructura o testing.
...
- `Makefile` — targets de testing.
- `.github/workflows/*` — matriz y comportamiento de CI.
- `tests/` — comportamiento que realmente se prueba.
- herramientas de coverage — configuración efectiva de cobertura.
- `ARCHITECTURE.md` — arquitectura y responsabilidades.
- `INSTALL.md` — instalación y configuración.
- Código y tests — comportamiento implementado definitivo ante contradicciones.
