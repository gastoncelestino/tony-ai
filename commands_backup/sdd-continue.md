...
1. Run `/sdd-continue [change]` (OpenCode slash command) — this delegates to the native dispatcher when artifact store is `openspec` or `hybrid`. When the session artifact store is `tonymem`, do NOT invoke any external binary; resolve status entirely from tonymem (`mem_search` + `mem_get_observation` on the change's topic keys) using the manual status schema in `~/.config/opencode/skills/_shared/sdd-status-contract.md`. If no active change is found and `$ARGUMENTS` is missing and more than one active change exists, ask the user to choose and STOP. Do not guess.
