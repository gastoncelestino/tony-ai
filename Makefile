...
.PHONY: test test-all check-prompts check-test-deps check-test-discovery check-coverage-deps test-python test-ts test-kernel coverage coverage-python coverage-ts verify-qdrant verify-sdd-flow docker-up docker-down clean bootstrap health validate-config
...
check-prompts:
	@set -eu; test ! -d prompts/generated || { echo "ERROR: prompts/generated no debe existir en dev"; exit 1; }; test ! -f prompts/agents/includes/phase-manifest.json || { echo "ERROR: phase-manifest.json no debe existir en dev"; exit 1; }; grep -q 'prompts/agents/tony-orchestrator.md' opencode.json; for phase in sdd-init sdd-onboard sdd-explore sdd-propose sdd-spec sdd-design sdd-tasks sdd-apply sdd-verify sdd-archive; do grep -q "prompts/sdd/$$phase.md" opencode.json; done; echo "✓ Phase prompts are plain, source-controlled, and non-generated"
