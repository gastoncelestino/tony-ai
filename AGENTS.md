...
- For a new execution with no existing SDD state, the first delegated Task MUST be the decomposition bootstrap: `description="decompose task graph"`, `subagent_type="explore"`, `command="tony:bootstrap-decompose"`. Its `prompt` MUST contain the complete user objective, including every requested deliverable and constraint; do not emit one `task()` call per user-written step before bootstrap.
- The bootstrap prompt is the only place where the complete objective is decomposed. Treat the returned TaskSet as authoritative: do not invent, rename, reorder, or manually execute tasks in the orchestrator context.
